...
namespace planning
{
    ...
    Time Time::operator-(const Time& t) const
    {
        ...
        int totalMinutes = result.hour * 60 + result.min - t.hour * 60 - t.min;
    ...
    istream& operator>>(istream& entree, Time& t)
    {
        ...
        min = stoi(line); //stoi = convert en int 
    ...
    {
        // Retourner l'heure et les minutes sous le format "hh:mm"
        return to_string(this->hour) + ":" + to_string(this->min);
    }
